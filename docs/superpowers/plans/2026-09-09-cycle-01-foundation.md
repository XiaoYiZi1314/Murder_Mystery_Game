...
- [ ] 创建BOSS使用受控CLI、显式秘密输入与单次初始化保护，无默认生产口令；员工API中BOSS可创建/维护DM与店长、按D11冻结（2026-09-10）店长可创建/维护DM，不开放代登录或任意顾客提权。
...
**验收：** 顾客不能构造员工请求；DM不能创建员工；店长仅可创建/维护DM（D11 2026-09-10冻结，范围不含店长/BOSS，不得变更自身角色或操作BOSS）；BOSS可创建DM与店长；被禁用员工的旧Cookie失效；不存在代登录入口/接口/内部服务捷径。
