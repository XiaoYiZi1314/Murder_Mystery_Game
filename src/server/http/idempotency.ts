  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (existing) {
      if (existing.requestHash !== requestHash) {
        throw conflict(ApiCodes.IDEMPOTENT_CONFLICT, "相同幂等键但请求内容不一致");
      }
      if (existing.status === "completed") {
        return { ...((existing.responseJson as IdempotentResult | null) ?? { status: 200, message: "OK", data: null }), replayed: true };
      }
    ...
    try {
      ...
      if (raced && raced.requestHash === requestHash && raced.status === "completed") {
        return { ...((raced.responseJson as IdempotentResult | null) ?? { status: 200, message: "OK", data: null }), replayed: true };
      }
    ...
    await tx.idempotencyRecord.update({
      where: { id: recordId },
      data: { status: "completed", responseJson: toJsonable(result) as Prisma.InputJsonValue, completedAt: new Date() },
    });
